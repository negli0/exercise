...
static void child_fn(int id, struct timespec *buf, int nrecord, unsigned long 
		nloop_per_resol, struct timespec start)
{
	int i;
	for (i = 0; i < nrecord; i++) {
		struct timespec ts;
...
{   
	...
	if (total % resol) {
		fprintf(stderr, "<resol>(%d) should be multiple of <resolution>(%d) \n",
				total, resol);
		exit(EXIT_FAILURE);
	}
